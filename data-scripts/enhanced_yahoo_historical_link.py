#!/usr/bin/env python3
"""
🌙 Enhanced Yahoo Finance Historical Data Fetcher - LINK Data 🚀
Fetches maximum years of LINK historical data with proper OHLCV format for Bobby's backtesting framework
...
# ====== Configuration ======
TICKER = 'LINK-USD'           # Yahoo Finance ticker symbol for Chainlink
...
def validate_data_quality(df: pd.DataFrame) -> Dict:
    ...
    # For crypto (LINK trades 365 days)
...
def main():
    ...
    print("🚀 Enhanced Yahoo Finance Historical Data Fetcher - LINK Backtesting.py Compatible")
    print("=" * 70)
    ...
    try:
        ...
        annual_volatility = daily_volatility * np.sqrt(365)  # 365 for crypto
        print(f"  Daily volatility: {daily_volatility:.2%}")
        print(f"  Annual volatility: {annual_volatility:.2%}")

        # Calculate Sharpe ratio (assuming 0% risk-free rate)
        annual_return = (df['close'].iloc[-1] / df['close'].iloc[0]) ** (365/len(df)) - 1
...
if __name__ == "__main__":
    data = main()
    if data is not None:
        print("\n✨ Enhanced LINK data fetching complete! Ready for backtesting.py 🚀")