"""
ETH Trend-Following Strategy Backtesting System V2
==================================================

This is an improved version that generates more trades by:
- Making entry conditions more flexible
- Adding alternative entry signals
- Reducing over-restrictive filters
...
class ETHTrendBacktesterV2:
    """
    Improved ETH trend-following strategy backtester with more flexible entry conditions
    """
    ...
    def __init__(self, initial_capital=10000, risk_per_trade=0.02):
        ...
        # Strategy parameters - more flexible
        self.sma_short = 10
        self.sma_long = 50
        self.rsi_period = 14
        self.rsi_overbought = 75  # Increased from 70 to allow more entries
        self.rsi_exit_overbought = 85  # Increased from 80
    ...
    def generate_signals(self):
        """
        Generate buy/sell signals with more flexible entry conditions
        """
        print("🎯 Generating trading signals with flexible entry conditions...")
        ...
        for i in range(self.sma_long, len(self.data)):
            ...
            # Entry Signal: Multiple entry conditions (more flexible)
            if position == 0:
                signal_type = ''
                
                # Condition 1: SMA crossover (primary signal)
                ...
                # Condition 3: Price above both SMAs + MACD bullish
                price_above_smas = current_price > sma_short and current_price > sma_long
                macd_bullish = macd > macd_signal
                
                # Condition 4: RSI not overbought + price momentum
                rsi_filter = rsi < self.rsi_overbought
                price_momentum = current_price > self.data['close'].iloc[i-5:i].mean()  # Price above 5-day average
                
                # Entry logic: Multiple ways to enter
                if sma_crossover and rsi_filter:
                    # Primary: SMA crossover + RSI filter
                    signal_type = 'SMA_CROSSOVER'
                elif macd_crossover and rsi_filter:
                    # Secondary: MACD crossover + RSI filter
                    signal_type = 'MACD_CROSSOVER'
                elif price_above_smas and macd_bullish and rsi_filter:
                    # Tertiary: Price above SMAs + MACD bullish + RSI filter
                    signal_type = 'MOMENTUM'
                elif sma_crossover:
                    # Fallback: Just SMA crossover (most permissive)
                    signal_type = 'SMA_ONLY'
                ...
                if signal_type:
                    ...
                    self.data.loc[i, 'signal_type'] = signal_type
            ...
            # Exit Signals: Multiple exit conditions
                ...
                # Additional exit: MACD turns bearish
                macd_bearish = macd < macd_signal
                
                if sma_crossunder or rsi_exit or stop_hit or take_profit_hit or macd_bearish:
                    position = 0
    ...
    def print_performance_summary(self):
        ...
        print("\n" + "="*60)
        print("📊 ETH TREND-FOLLOWING STRATEGY PERFORMANCE SUMMARY V2")
        print("="*60)
    ...
    def plot_results(self):
        ...
        fig.suptitle('ETH Trend-Following Strategy Backtest Results V2', fontsize=16, fontweight='bold')
    ...
    def run_backtest(self, data_file):
        ...
        """
        print("🚀 Starting ETH Trend-Following Strategy Backtest V2 (More Flexible)")
        print("="*70)
...
def main():
    ...
    backtester = ETHTrendBacktesterV2(
    ...
    if success:
        ...
        print("💡 This version should generate more trades with flexible entry conditions")
    else:
